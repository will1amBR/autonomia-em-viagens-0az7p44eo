...
routerAdd('POST', '/backend/v1/duress-silent-alert', (c) => {
  ...
  const userName = user
    ? user.getString('name') || user.getString('email') || 'Usuário'
  ...
  const approx =
    body.locationApprox ||
    body.approxLocation ||
    (lat && lng ? `GPS (${lat.toFixed(4)}, ${lng.toFixed(4)})` : 'Localização não informada')
  const battery =
  ...
  try {
    ...
    duressRecord.set('location_address', approx)
    duressRecord.set('device_info', deviceInfo)
    duressRecord.set('notified_guardians_count', 3)
    duressRecord.set('notified_police', true)
    duressRecord.set('status', 'dispatched')
    duressRecord.set('timestamp', new Date().toISOString())
    $app.save(duressRecord)
    console.log(
      `[Duress Alert] Alerta silencioso gravado com sucesso para usuário: ${finalUserId} (método: ${method})`,
    )
  } catch (err) {
    console.log('[Duress Alert] Erro ao gravar duress_alert:', err)
  }

  // 4.1 Também salvar no presence_logs como sinal de coação para histórico unificado
  try {
    const presenceColl = $app.findCollectionByNameOrId('presence_logs')
    const pRecord = new Record(presenceColl)
    pRecord.set('user_id', finalUserId)
    if (tripId) pRecord.set('trip_id', tripId)
    pRecord.set('event_type', 'duress_signal')
    if (lat !== null) pRecord.set('location_lat', lat)
    if (lng !== null) pRecord.set('location_lng', lng)
    pRecord.set('location_name', approx)
    pRecord.set('device_info', deviceInfo)
    pRecord.set('is_duress', true)
    pRecord.set('notes', `Sinal sob ameaça acionado via ${method}`)
    pRecord.set('timestamp', new Date().toISOString())
    $app.save(pRecord)
  } catch (pErr) {
    console.log('[Duress Alert] Erro ao gravar presença do duress:', pErr)
  }
  ...
  try {
    ...
    console.log(
      '[Duress Alert] Mail client indisponível ou SMTP ausente (simulado):',
