    ...
    for (let i = 0; i < tripsToCheck.length; i++) {
      ...
      if (stage > 0) {
        ...
        try {
          ...
          notificationRecord = new Record(absenceColl)
          notificationRecord.set('user_id', userId)
          notificationRecord.set('trip_id', trip.id)
          notificationRecord.set('stage', stage)
          ...
          if (stage === 1) {
            actionSummary = `Etapa 1: Notificação e lembrete amigável enviado à viajante (${userEmail}).`
            targets.push(userEmail)
          } else if (stage === 2) {
            actionSummary = `Etapa 2: Alerta discreto enviado ao Guardião Primário de Emergência.`
            const primaryG = guardians.find(g => g.getString('role_type') === 'emergency') || guardians[0]
            if (primaryG) targets.push(primaryG.getString('email'))
          } else if (stage === 3) {
            actionSummary = `Etapa 3: Alerta prioritário enviado a todos os guardiões e dados do anfitrião (${hostName} - ${hostPhone}) consolidados.`
            guardians.forEach(g => targets.push(g.getString('email')))
          } else if (stage === 4) {
            actionSummary = `Etapa 4: Protocolo consular e rede completa de segurança acionados.`
            guardians.forEach(g => targets.push(g.getString('email')))
          }

          notificationRecord.set('actions_taken', actionSummary)
          notificationRecord.set('notes', `Alvo(s): ${targets.filter(Boolean).join(', ')} | Última localiz.: ${lastPresenceLocation}`)
        ...
        try {
          ...
          if (stage === 1 && userEmail) {
            ...
            const targetGuardians = (stage === 2)
              ? guardians.filter(g => g.getString('role_type') === 'emergency').slice(0, 1)
              : guardians
